...
class GeminiReasoner(LanguageModel):
    def __init__(
        ...
        if not self.api_key:
            raise ValueError(
                "GOOGLE_API_KEY is required. Set env var or pass api_key=..."
            )
    ...
    def _to_contents(
        self, messages: List[Message], tools_schema: Dict[str, Any] | None
    ) -> List[dict]:
        contents: List[dict] = []
        tool_desc = (
            f"Available tools: {list(tools_schema.keys())}"
            if tools_schema
    ...
    async def generate(
        self, messages: List[Message], tools_schema: Dict[str, Any] | None = None
    ) -> str:
        ...
        config = genai.GenerationConfig(**config)
        try:
            ...
            e = str(e).replace("\"","'")
            return f'{{"type":"final","answer":"Gemini error: {e}"}}'
